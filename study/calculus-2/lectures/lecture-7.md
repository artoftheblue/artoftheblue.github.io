...
:name: feasible-set
Множество $\fset\subset\RR^n$ называется {bluehighlight}`допустимым`, если 

1. $\fset$ --- [ограниченное](#bounded-set).
2. Граница $\partial \fset$ --- [множество меры нуль по Лебегу](#null-measure).
```
...
где $I$ --- произвольный [брус](#block) в $\RR^n\colon \fset\subset I$.
...
Если $\exists$ такое $\mathcal{I}<\infty$, то $f\in\mathcal{R}(\fset)$.
```

:::{prf:proposition} Корректность определения
Обоснуем корректность такого определения.
1. Изначально $f$ не определена вне $\fset$.
2. Вне $\fset$ можно доопределить $f$ как угодно, так как $f\cdot\xi_D=0$ при $x\not\in \fset$.
3. Определение не зависит от выбора бруса $I$.
:::

:::{prf:proof}
:nonumber:
$\fset\subset \RR^n$ --- [допустимое множество](#bounded-set). <br>
$I_1, I_2$ --- замкнутые брусы: $\fset\subset I_1$, $\fset\subset I_2$, тогда интегралы $\displaystyle\int\limits_{I_1\supset \fset}f\cdot \chi_D\d \vec x$ и $\displaystyle\int\limits_{I_2\supset \fset}f\cdot \chi_D\d \vec x$ либо одновременно $\exists$ и равны; либо оба $\cancel\exists$.

---

$I=I_1\cap I_2\colon \fset\subset I$

$f\cdot\chi_D\in\mathcal{R}(I_1)\implies$ по критерию Лебега $f\cdot\chi_D$ ограничена на $I_1\implies f\cdot\chi_D$ ограничена на $\fset\implies f$ ограничена на $\fset\implies f\cdot\chi_D$ ограничена на $I_2$.

Получаем, что по [критерию Лебега](#Lebegue-criterion) $f\cdot\chi_D$ непрерывна [почти всюду](#almost-everywhere) на $I_1\implies f\cdot\chi_D$ непрерывна почти всюду на $\fset\implies f$ непрерывна почти всюду на $\fset$.

В худшем случае для $f\cdot\chi_D$ на $I_2$ добавятся разрывы на $\partial \fset\implies f\cdot\chi_D$ непрерывна почти всюду на $I_2$.

**Покажем, что равны**:

$\TT_i$ --- [разбиение](#block-decomposition) на $I_i\colon \TT_1$ и $\TT_2$ совпадают на общей части $I_1\cap I_2$. <br>
$\xi^i$ --- [отмеченные точки](#marked-points) $I_i\colon$ совпадают на общей части.

$$\begin{align*}
  \sigma(f\cdot\chi_D,\TT_1,\xi^1)&=\sum_jf\chi_D(\xi^1_j)|I^1_j|=\sum_j f\cdot\chi_D(\xi_i)|I_j|\\
  &=\sum_jf\cdot\chi_D(\xi_j^2)|I^2_j|=\sigma(f\cdot\chi_D,\TT_2,\xi^2)
\end{align*}$$
:::
...
```{prf:theorem} Фубини
:name: foubini
$I_x\subset\RR^n,I_y\subset\RR^m, I_x\times I_y\subset \RR^n \times \RR^m = \RR^{n+m}$ --- [замкнутые брусы](#closed-block). <br>
...
$\displaystyle f(x, y)=\frac{x^2-y^2}{(x^2+y^2)^2}$ на $\fset=[-1, 1]^2$ <br>
...
\int\limits_{-1}^1\int\limits_{-1}^1\frac{x^2-y^2}{(x^2+y^2)^2}\d x\d y
  ...
  &=-2\left[\arctg y\right]_{-1}^1=-2\left(\arctg 1 - \arctg (-1)\right)\\
  &=-2\left(\frac{\pi}{4} - \left(-\frac{\pi}{4}\right)\right)=-2\left(\frac{\pi}{4} + \frac{\pi}{4}\right)\\
  &=-2\left(\frac{2\pi}{4}\right)=-2\left(\frac{\pi}{2}\right)=-\pi
\end{align*}
$$
...
2. Будем пользоваться $f\in\mathcal{R}(I_x\times I_y)\implies \exists$ [(1)]( #hash-lect8-1) + [по критерию Дарбу](#darbu-riemann-integration-criterion) <br>
  ...
  $$\begin{align*}
    ...
    &=\Sl(g(x),\TT_x)\leq\Su(g(x),\TT_x)\leq\underbrace{\ldots}_{\text{так же для }\sup}\\
    &\leq\sum_i\sup_{x\in I_i^x} g(x) \cdot |I^x_i| \leq \ldots \leq \Su(f,\TT_{x,y})\\
  \end{align*}$$
...
$M_1,M_2\subset\RR^n$ --- [открытые](#open-set) множества. <br>
$\varphi\colon M_1\to M_2$ --- биективное отображение. <br>
$\varphi,\varphi^{-1}\in \diffclass$ --- непрерывно дифференцируемы. <br>
$\fset\colon\overline{\fset}\subset M_1$ --- [допустимое множество](#feasible-set). <br>
$f\colon\varphi(\fset)\mapsto\RR$. <br>
$f(x)\in\mathcal{R}(\varphi(\fset))\iff f(\varphi(t))\cdot|\det\mathcal{J}_\varphi(t)|\in\mathcal{R}(\fset)$, причём 

$$\int\limits_{\varphi(\fset)}f(x)\d x=\int\limits_{\fset}f(\varphi(t))\cdot|\det\mathcal{J}_\varphi(t)|\d t\qquad \mathcal{J}_\varphi(t)=\begin{pmatrix}

...
:name: closed-block-compactness
[Замкнутый брус](#closed-block)` $I\subset\RR^n$ --- [компакт](#compact).
:::

:::{prf:proof}
**(От противного)** Рассмотрим замкнутый брус $I=[a_1,b_1]\times\ldots\times[a_n, b_n]$.

1. Предположим, что $I$ не компакт, тогда $\exists$ какое-то покрытие [открытыми множествами](#open-set) $\{A_\alpha\}_{\alpha\in Z}$, где $Z$ --- какое-то множество, где $I\subset\{A_\alpha\}$, не позволяющее выделить конечное подпокрытие.
...
3. Повторим процесс деления ещё и ещё и получим систему вложенных брусов. $$I \supset I_1 \supset I_2 \supset \ldots$$ Таким образом, по теореме о вложенных отрезках $\exists !$ предельная точка при $n\to\infty$
  $$\displaystyle \vec a=(a_1,\ldots,a_n)\in\bigcap^\infty_{i=1} I_i, \exists! \vec a=\in\RR^n\colon a\in I_n$$
  % todo: add link to теореме о вложенных отрезках

4. Таким образом, $\exists \alpha_0, \vec a \in I\implies A_{\alpha_0}\colon \vec a \subset A_{\alpha_0}, A_{\alpha_0}$ --- открытое $\implies\exists r>0\colon \ball_r(a)\subset A_{\alpha_0}$.
...
$\KK$ --- [компакт](#compact) в $\RR^n\iff \KK$ --- [замкнуто](#closed-set) и [ограничено](#bounded-set).
:::
...
2. **Замкнутость** (От противного)
    ...
    $\KK$ --- компакт, но не замкнуто, то есть он не содержит **ВСЕ** предельные точки:
...
1. $\KK$ ограничено $\implies \exists r>0, \KK \subset \ball_r(0)$. Пусть $\exists$ замкнутый брус $I=[-r,r]^n\supset \KK$.

2. Пусть $\{A_\alpha\}$ --- покрытие открытыми множествами $\KK$, тогда $\{A_\alpha\}\cup\{\RR^n\setminus \KK\}$ --- покрытие открытыми множествами $I$, так как $\KK$ замкнуто, но $I$ --- брус в $\RR^n \implies I$ --- компакт.
   
3. Из покрытия $\{A_{\alpha_i}\}^s_{i=1}\cup\{\RR^n\setminus \KK\}\supset I$ всегда можно выделить конечное подпокрытие. Тогда, удаляя $\RR^n\setminus \KK$, получим конечное подпокрытие из $\{A_\alpha\}$ для $\KK$ (так как $\{A_\alpha\}$ --- произвольное покрытие открытыми множествами $\KK$).
:::
...
$\KK\subset \RR^n$ --- компакт. $f\colon \KK\mapsto\RR$. $f\in \contclass(\KK)$. <br>
$f$ --- [ограниченная](#bounded-set) на $K$ и достигает своего наибольшего ({bluehighlight}`супремум`, $\sup$) и наименьшего ({bluehighlight}`инфимум` $\inf$) значений.
```
...
1. $f$ --- {bluehighlight}`непрерывная в точке` $x_0$, если $\forall \ve>0,\exists \delta_\ve>0\colon \forall x\in\ball^M_{\delta_\ve}(x_0)\implies |f(x)-f(x_0)|<\ve$.

2. $f$ --- {bluehighlight}`непрерывная на множестве`, если $f$ непрерывна во всех точках этого множества.
:::
...
1. **Ограниченность** (От противного)
   ...
   $\KK$ --- [компакт](#compact) $\implies K$ [замкнуто](#closed-set) и ограничено $\implies\{\vec x^k\}$ ограничено $\implies \exists c,||\vec x^k||\leq c,\implies\{x^k\}^\infty_{k=1}$ ограничена
   ...
   $\{x^{k_{j_n}}\}^\infty_{j_n=1},x^{k_{j_n}}\to(a_1,\ldots, a_n)=\vec a \implies x^{k_{j_n}}\xrightarrow{j_n\to\infty} a$, то есть $a$ предел $\KK$.

   Так как $\KK$ --- компакт $\implies \KK$ замкнуто $\implies \KK$ содержит все свои [предельные точки](#limit-point) $\implies a \in \KK + f\in \contclass(\KK)\implies f$ --- [непрерывная в точке](#continuity) $\vec a \implies$
...
:::{seealso} Замечание
Из критерия компактности $\implies$ если $K$ --- [компакт](#compact), $L\subset K$ --- [замкнуто](#closed-set), то $L$ --- компакт.
:::

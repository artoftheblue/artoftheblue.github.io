...
$$I=\langle a_1, b_1\rangle \times\dots\times\langle a_n,b_n\rangle$$

* $\langle a,b\rangle$ --- отрезок, интервал, полуинтервал.
```

```{prf:definition} Мера бруса
:label: block-measure
...
:::{div}
:label: block-measure-properties
## Свойства меры бруса в $\mathbb{R}^n$

```{card}
:header: 1. {bluehighlight}`Однородность` 
...
```{card}
:header: 2. {bluehighlight}`Аддитивность`

Пусть $\displaystyle I, I_1,\dots,I_k\colon I=\bigcup^k_{i=1}I_i$. $I_1,\dots,I_k$ не имеют общих {bluehighlight}`внутренних точек` $\displaystyle\implies |I|=\sum^k_{i=1}|I_i|$
```

```{card}
:header: 3. {bluehighlight}`Монотонность`

$\displaystyle I\subset\bigcup^k_{i=1}I_i$ покрыт конечной системой брусов $\displaystyle\implies |I|\leq\sum^k_{i=1}|I_i|$
```
:::

## Разбиения

```{prf:definition} Разбиение бруса
:label: block-decomposition
Пусть задан брус $I=[a_1, b_1]\times\ldots\times[a_n,b_n]$ --- замкнутый невырожденный брус и $\displaystyle I=\bigcup_{i=1}^kI_i$, где $I_i$ попарно не имеет общих внутренних точек, тогда набор таких более мелких брусов $\mathbb{T}=\{I_i\}^k_{i=1}$ будем называть {bluehighlight}`разбиением бруса` $I$.
```
...
```{prf:definition} Интегральная сумма Римана
:label: riemann-integral-sum
...
```{prf:definition} Интегрируемая функция
:label: integrable-function
Будем говорить, что функция $f\colon I\mapsto\RR$ {bluehighlight}`интегрируема (по Риману)` на замкнутом брусе $I$, если $\exists A\in\mathbb{R},\forall\varepsilon>0,\exists\delta>0,\forall(\mathbb{T},\bxi)\colon \triangle_{\mathbb{T}}<\delta$ верно, что $|\sigma(f,\mathbb{T},\xi)-A|<\varepsilon$. Тогда
...
$$f=\begin{cases}
1, & \forall i=1,\dots,n, & x_i\in\mathbb{Q}\\
0, & \text{иначе}
\end{cases}$$

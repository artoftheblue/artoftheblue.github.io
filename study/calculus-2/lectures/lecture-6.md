...
Пусть $I$ --- [замкнутый брус](#closed-block). $f\colon I\mapsto\RR$. $\TT=\{I_i\}^k_{i=1}$ --- [разбиение бруса](#block-decomposition) $I$.
...
2. Пусть $\tilde\TT$ --- [измельчение разбиения](#decomposition-shredding) $\TT$, тогда 
...
$I\subset \RR^n$ --- [замкнутый брус](#closed-block), $f\colon I\mapsto\RR^n$ --- [ограниченная](#bounded-set). <br>
...
1. Если $f$ ограничена на $I$, то $\exists c>0,\forall x\in I,|f(x)|<c$.

:::{div}
:label: darbu22
2. По определению [интеграла Дарбу](#darbu-integrals) $\displaystyle \Il=\sup_\TT\Sl(f,\TT)$, то $\forall\ve>0,\exists\TT=\{I_i\}^k_{i=1}\colon$

  $$\boxed{\Il-\ve<\Sl(f,\TT)\leq\Il<\Il+\ve}$$
:::

3. Пусть $\displaystyle G=\bigsqcup_{i=1}^{k_1}\partial I_i$ --- объединение границ брусов (оно дизъюнктное, то есть без повторов). $\implies G$ --- множество [меры нуль по Лебегу](#null-measure) (т. к. границы --- множества меры нуль).

4. Пусть $\tilde \TT$ --- произвольное [разбиение](#block-decomposition) $I\colon\tilde\TT=\{\tilde I_j\}^m_{j=1}$. Рассмотрим две кучки брусов: $$\boxed{A=\{\tilde I_j\in \tilde\TT \colon \tilde I_j\cap G\neq\varnothing\},\quad B=\tilde\TT\setminus A}$$ $$\implies\forall\ve>0,\exists \delta>0\colon\forall\tilde\TT\colon\Delta_{\tilde\TT}<\delta\hookrightarrow\boxed{\sum_{\tilde I_j\in A}|\tilde I_j|<\ve}$$ по определению множества меры нуль, так как $A$ --- покрытие  $G$ замкнутыми брусами, $G$ --- множество меры нуль по Лебегу.
...
$I\subset\RR^k$ --- [замкнутый брус](#closed-block), $f\colon I\to\mathcal{R}$. <br>
$f\in\mathcal{R}(I)\iff f$ --- [ограниченная](#bounded-set) на $I$ и $\underline{\mathcal{I}}=\overline{\mathcal{I}}$.
```
...
$f\in\mathcal{R}(I)\implies$ по [необходимому условию интергируемости по Риману](#required-condition-integral) $f$ ограничена на $I$. **Идея:** показать, что $\Il=\II$ и $\Iu=\II \implies \boxed{\Il=\Iu}$.
   1. $f\in\mathcal{R}(I)\implies\exists \II,\forall\\ve>0,\exists\delta >0\colon\forall(\TT,\xi)\colon\Delta_\TT<\delta$.

   :::{div}
   :label: darbu-point-2
   2. $$|\underbrace{\sigma(f,\TT,\xi)}_\sigma-\II|<\frac{\\ve}{3}$$
   ...
   3. $$\Il=\sup_{\TT}\Sl(f,\TT)=\lim_{\Delta\to0}\Sl(f,\TT)\implies|\Il-\Sl|<\frac{\\ve}{3}$$

   4. $$\Sl(f,\TT)=\inf_\bxi\sigma(f,\TT,\bxi)\implies \forall \ve>0,\exists \xi\colon |\Sl-\sigma|<\frac{\ve}{3}$$

   5. Симметричное доказательство для $|\Iu-\II|$ как в [пункте 2](#darbu-point-2). Мы доказали равенство с $\II$ для каждой [суммы Дарбу](#darbu-sums).

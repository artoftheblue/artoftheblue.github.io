...
description: Скалярное произведение. Ортогональность и ортонормированность. Линейная независимость. Ортогонализация Грама-Шмидта. Теорема о непрерывности скалярного произведения. Ряды Фурье. Коэффициенты Фурье.
numbering:
  enumerator: 15.%s
...
:::{card}
$V$ --- линейное векторное пространство над полем $\FF=(\RR, \CC)$ <br>
$f_1,\ldots,f_n$ --- векторы или функции, задано $X\in\RR^n$ <br>
$\lambda_i\in\FF$
...
:label: orthogonal
$V$ --- векторное пространство над $\FF$ с заданным [скалярным произведением](#dot-product) $\langle \cdot, \cdot \rangle$, тогда {bluehighlight}`векторы` $x, y\in V$ называются {bluehighlight}`ортогональными`, если $\langle x, y \rangle=0$
:::

:::{prf:definition} Ортогональная система векторов
:label: orthogonal-system
$V$ --- векторное пространство над $\FF$ с заданным [скалярным произведением](#dot-product) $\langle \cdot, \cdot \rangle$, $\{e_k\}$ --- {bluehighlight}`система векторов` из $V$ называется {bluehighlight}`ортогональной`, если $\forall e_i, e_j\in\{e_k\}$, скалярное произведение $\langle e_i, e_j\rangle=0$ при $i\neq j$.
:::
...
$V$ --- векторное пространство над $\FF$ с заданным [скалярным произведением](#dot-product) $\langle \cdot, \cdot \rangle$, $\{e_k\}$ --- система векторов в $V$ называется {bluehighlight}`ортонормированной (ОНС) системой векторов`, если $\forall e_i, e_j \in \{e_k\}$:
...
{bluehighlight}`Произвольная` система векторов $\{e_k\}$ из $V$ называется {bluehighlight}`линейно независимой`, если [линейно независима](#linearly-dependent-end) каждая её конечная подсистема.
:::

:::{prf:example}
$V$ --- векторное пространство функций: <br>
...
$\{1,\sin nx, \cos nx\}_{n\in\NN}$ --- [ортогональная система](#orthogonal-system).
...
:label: gram-schmidt
Пусть $a_1, a_2, \ldots$ --- система [линейно независимых](#linearly-dependent) векторов.
...
Пусть $\langle \cdot, \cdot \rangle\colon V\times V\mapsto \CC$ --- [скалярное произведение](#dot-product) на $V$ над $\CC$. Тогда
...
:::{prf:proof}
1. $$\begin{align*}
       x\in V\mapsto x_0\in V, &\forall \ve >0, \exists \delta=\ve\colon\\
    &\forall x \in \overset\circ\ball_\delta(x_0)\colon ||x-x_0||<\ve, \quad (x_0, y_0)\in V
    \end{align*}$$

   $$\begin{align*}
   |f(x, y)- f(x_0, y_0)|&=|\underline{\langle x, y \rangle} + \underline{\underline{\langle x_0, y\rangle}} - \underline{\langle x_0, y\rangle} \\
   &\qquad\qquad+\  \langle x, y_0 \rangle - \langle x, y_0 \rangle - \underline{\underline{\langle x_0, y_0 \rangle}} |\\
...
2. $$x=\sum^\infty_{i=1}x_i,\quad S_n=\sum^n_{i=1}x_i$$
    ...
    $$
    \langle x, y \rangle=\left\langle \sum^n_{i=1} x_i, y \right\rangle+\left\langle \sum^\infty_{i=n+1}x_i, y\right\rangle=\sum^n_{i=1}\langle x_i, y\rangle + \langle x - S_n, y\rangle
    $$
...
3. $\{e_i\}$ --- [ортонормированная система векторов](#orthonormal-system). $\langle e_i, e_j \rangle =\delta_{ij}$.
...
:::{card}
$\ds x=\sum_i x_ie_i$, $\{e_i\}$ --- ортогональная система векторов.
...
$V$ --- линейное пространство со [скалярным произведением](#dot-product) $\langle \cdot,\cdot \rangle$ и $\{e_i\}$ --- ненулевая ортогональная система векторов $V$, то любому $x\in V$ можно сопоставить ряд 
